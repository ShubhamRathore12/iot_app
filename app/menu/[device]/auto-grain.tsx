...
export default function AutoGrainScreen() {
  ...
  const config = useMemo(() => getGrainHMIConfig(device || ''), [device]);
  const isSpecialMachine = useMemo(
    () => SPECIAL_MACHINES.includes(device || ''),
    [device],
  );
  ...
  const isGrainChillingMode =
    String(data?.Grain_chilling_mode)?.toLowerCase() === 'true';
  ...
  const getT1T0Value = useCallback(() => {
    ...
    if (isSpecialMachine && isGrainChilling) {
      return (
        data.T0_set_point ||
    ...
    return data.T1_set_point || data.T1_temp_mean || data.T1_SET_POINT || '--';
  }, [data, device, isSpecialMachine, isGrainChilling]);
  ...
  }, [data, device, isSpecialMachine]);

  const t1t0Label = device?.includes('GTPL-061')
    ? 'T1 Set Point'
    : isSpecialMachine
      ? 'T0'
      : 'T1';

  const tDeltaLabel = device?.includes('GTPL-061')
    ? 'T0 - T1'
    : isSpecialMachine
      ? 'T Delta'
      : 'TH - T1';
      ...
      >
        ...
        <View style={styles.grid}>
          <DataCard
            ...
            index={temperatureCards.length + controlCards.length}
          />
          <DataCard
            label="HP Value"
            ...
            index={temperatureCards.length + controlCards.length + 1}
          />
        </View>
