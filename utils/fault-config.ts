...
export function getFaultCodesForMachine(machineName: string) {
  ...
  // Fault codes only for S7-200 and E-series machines
  // AP machines (132, 136, 139, 142, 143, 123) and T-series have no fault codes

  // S7-200 machines (Germany)
  if (config.type === "S7-200") {
    return FAULT_CODES["S7-200"];
  }

  // E-series: GPL-115 (GTPL-30, 115, 119, 120)
  if (
  ...
  // All other machines (AP, T-series, etc.) - no fault codes
  return [];
}
...
export function isActiveTag(value: any): boolean {
  if (value === null || value === undefined || value === "" || value === false || value === 0 || value === "0") return false;
  if (value === true || value === 1) return true;
  const normalized = value.toString().toLowerCase().trim();
  return normalized === "true" || normalized === "tr" || normalized === "1" || normalized === "yes" || normalized === "on";
}
